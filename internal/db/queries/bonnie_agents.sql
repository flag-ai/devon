...
SELECT * FROM devon_bonnie_agents ORDER BY name;

-- name: GetBonnieAgent :one
SELECT * FROM devon_bonnie_agents WHERE id = $1;

-- name: GetBonnieAgentByName :one
SELECT * FROM devon_bonnie_agents WHERE name = $1;

-- name: CreateBonnieAgent :one
INSERT INTO devon_bonnie_agents (name, url, token, status)
VALUES ($1, $2, $3, $4)
RETURNING *;

-- name: UpdateBonnieAgent :one
UPDATE devon_bonnie_agents
SET name = $2, url = $3, token = $4, updated_at = now()
WHERE id = $1
RETURNING *;

-- name: DeleteBonnieAgent :exec
DELETE FROM devon_bonnie_agents WHERE id = $1;
